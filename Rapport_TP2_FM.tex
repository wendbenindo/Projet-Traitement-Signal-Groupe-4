...
\begin{titlepage}
    ...
    {\LARGE \textbf{TP 2 : Modulation FM}}\\
    \vspace{1cm}
    {\Large Traitement du Signal}\\
    \vspace{2cm}
...
\begin{lstlisting}[caption=Démodulation du signal bruité]
x2_noisy = fmdemod(s_FM_noisy, fc, fs, delta_f);
\end{lstlisting}

\textbf{Commentaire du résultat :}

\begin{itemize}
    \item Pour un SNR élevé (> 20 dB) : Le signal démodulé est très proche du signal original, le bruit est faible.
    
    \item Pour un SNR moyen (10-20 dB) : Le signal démodulé présente du bruit mais reste exploitable, la forme du signal est reconnaissable.
    
    \item Pour un SNR faible (< 10 dB) : Le signal démodulé est fortement bruité, la qualité se dégrade significativement.
\end{itemize}
...
\begin{lstlisting}[caption=Analyse du seuil de SNR]
...
for i = 1:length(SNR_values)
    ...
    MSE_values(i) = mean((x - x2_noisy).^2);
end
\end{lstlisting}
...
\begin{itemize}
    \item \textbf{Seuil de démodulation :} Le signal FM peut être correctement démodulé pour un SNR supérieur à environ 10-12 dB.
    
    \item \textbf{Effet de seuil :} En dessous de ce seuil, la qualité de démodulation se dégrade rapidement (effet de seuil FM).
    
    \item \textbf{Avantage de la FM :} Pour un SNR suffisant, la modulation FM offre une meilleure résistance au bruit que la modulation AM, grâce à l'effet de capture et à la possibilité d'utiliser des indices de modulation élevés.
\end{itemize}
