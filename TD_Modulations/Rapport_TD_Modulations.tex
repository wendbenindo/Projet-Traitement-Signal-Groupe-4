...
\begin{itemize}
    \item GO : 1052 m à 2000 m
    \item FM : de 87,5 MHz à 108 MHz
\end{itemize}

\subsection{Question 1 : Tableau fréquences et longueurs d'onde}

\textbf{Raisonnement :}

La relation entre fréquence et longueur d'onde est : $f = \frac{c}{\lambda}$ où $c = 3 \times 10^8$ m/s.

Pour GO : $f_{min} = \frac{3 \times 10^8}{2000} = 150$ kHz, $f_{max} = \frac{3 \times 10^8}{1052} \approx 285$ kHz

Pour FM : $\lambda_{max} = \frac{3 \times 10^8}{87{,}5 \times 10^6} \approx 3{,}43$ m, $\lambda_{min} = \frac{3 \times 10^8}{108 \times 10^6} \approx 2{,}78$ m
...
\subsection{Question 2 : Nombre d'émetteurs en GO}

\textbf{Raisonnement :}

Bande totale GO : $285 - 150 = 135$ kHz. Largeur par canal : 9 kHz.

Nombre d'émetteurs : $N = \left\lfloor \frac{135}{9} \right\rfloor = 15$

\begin{center}
\fbox{\parbox{0.7\textwidth}{
\centering
\textbf{Résultat :} $N = 15$ émetteurs
}}
\end{center}

\subsection{Question 3 : Fonctions contenant le signal modulant}

\textbf{a) En modulation AM :}

Signal : $s(t) = A_c[1 + m \cdot x(t)] \cos(2\pi f_c t)$

\begin{center}
\fbox{\parbox{0.8\textwidth}{
\centering
\textbf{Résultat :} $X(t) = A_c[1 + m \cdot x(t)]$ (amplitude)
}}
\end{center}

\textbf{b) En modulation FM :}

Signal : $s(t) = A_c \cos(2\pi f_c t + 2\pi k_f \int x(\tau) d\tau)$

\begin{center}
\fbox{\parbox{0.8\textwidth}{
\centering
\textbf{Résultat :} $\theta(t) = 2\pi f_c t + 2\pi k_f \int x(\tau) d\tau$ (phase)
}}
\end{center}
...
\subsection{Partie 1 : NFM ($\beta = 0{,}1$)}

Paramètres : $f_p = 100$ MHz, $f_m = 10$ kHz, $A_m = 1$ V, $R = 50$ $\Omega$

\subsubsection{Question 1 : Expression de $\theta(t)$}

\textbf{Raisonnement :}

$\theta(t) = 2\pi f_p t + \varphi(t)$ où $\varphi(t) = \beta \sin(2\pi f_m t)$

Avec $\beta = \frac{k A_m}{f_m}$, on obtient $k = \frac{\beta f_m}{A_m} = 1000$ Hz/V

\begin{center}
\fbox{\parbox{0.8\textwidth}{
\centering
\textbf{Résultat :} $\theta(t) = 2\pi f_p t + 0{,}1 \sin(2\pi f_m t)$
}}
\end{center}

\subsubsection{Question 3 : Puissance}

$P = \frac{A_p^2}{2R} = \frac{1}{100} = 10$ mW

\begin{center}
\fbox{\parbox{0.7\textwidth}{
\centering
\textbf{Résultats :} $P = 10$ mW $= 10$ dBm
}}
\end{center}

\subsubsection{Question 4 : Largeur canal}

$BW \approx 2 f_m = 20$ kHz
...
\subsection{Partie 2 : WFM ($\beta = 6$)}

\subsubsection{Question 1 : Coefficients de Bessel}

Pour $\beta = 6$, on garde $|J_n| > 0{,}1$ : 5 termes (n = 0 à 4)

\begin{center}
\fbox{\parbox{0.7\textwidth}{
\centering
\textbf{Résultat :} 9 raies significatives
}}
\end{center}

\subsubsection{Question 3 : Largeur canal}

$BW = 2(\beta f_m + f_m) = 2 \times 10 \times 7 = 140$ kHz

\begin{center}
\fbox{\parbox{0.6\textwidth}{
\centering
\textbf{Résultat :} $BW = 140$ kHz
}}
\end{center}
